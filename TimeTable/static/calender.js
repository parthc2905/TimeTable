// Get eventDates from embedded JSON in HTML
const eventScript = document.getElementById("event-data");
const eventDates = JSON.parse(eventScript.textContent);
...
const createButton = (text, isDisabled = false, type = 0) => {
  ...
  // Format this button's date to YYYY-MM-DD for comparison
  const btnDate = new Date(year, month + type, text);
  ...
  const btnDateStr = formatDate(btnDate);
  
  // Add 'event' class if the date is in the event list
  if (eventDates.includes(btnDateStr) && !isDisabled) {
