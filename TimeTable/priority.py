...
def tableGen(numberOfDays, numOflecure, numofsub, breaktime, namefaculty, nameSubject, lectureduraion, numofdiv, subject_priority = {
    "Math": 5,
    "Physics": 4,
    "Chemistry": 3,
    "Biology": 2,
        ...
        for day in range(total_days_per_week):
            ...
            for lecture in range(total_time//lecture_duration + 1):
                if lecture == (total_time//lecture_duration)//2:
                    ...
                    # Sort available subjects by priority (descending)
                    sorted_subjects = sorted(temp, key=lambda x: subject_priority.get(x, 1), reverse=True)
                    # Randomly select from top 3 priority subjects if possible
                    top_subjects = sorted_subjects[:min(3, len(sorted_subjects))]
                    rand = random.choice(top_subjects)
...
x = tableGen(5,4,5,45,['f1','f2','f3','f4','f5'],["Math","Physics","Chemistry","Biology","History"],60,3)
for i in x[0]:
    print(x[0][i])
    print()